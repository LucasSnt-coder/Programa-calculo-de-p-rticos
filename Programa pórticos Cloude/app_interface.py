...
class MainStructuralApp(ctk.CTk):
    def __init__(self):
        ...
        self.current_diagram = "structure"  # structure|N|V|M
    ...
    def _build_editor_tab(self):
        ...
        if self.structure_type == "frame":
            ...
            self._add_labeled_entry(load_elem_grid, "w2 (j):", "w2", row=1,
                                    default="0", form=self.elem_form)
            ctk.CTkLabel(left, text="w: carga distribuida transversal (eje local +y)",
        ...
        self.btn_view_structure = ctk.CTkButton(
            toolbar, text="Estructura", width=110,
            command=lambda: self._set_diagram_view("structure"))
        self.btn_view_structure.pack(side="left", padx=3)

        self.btn_view_axial = ctk.CTkButton(
            toolbar, text="Axial (N)", width=110,
            command=lambda: self._set_diagram_view("N"))
        self.btn_view_axial.pack(side="left", padx=3)

        if self.structure_type == "frame":
            self.btn_view_shear = ctk.CTkButton(
                toolbar, text="Cortante (V)", width=110,
                command=lambda: self._set_diagram_view("V"))
            self.btn_view_shear.pack(side="left", padx=3)

            self.btn_view_moment = ctk.CTkButton(
                toolbar, text="Momento (M)", width=110,
                command=lambda: self._set_diagram_view("M"))
            self.btn_view_moment.pack(side="left", padx=3)
    ...
    def _on_calculate(self):
        ...
        try:
            ...
            self._set_diagram_view("structure" if self.structure_type == "truss" else "M")
    ...
    def _redraw(self):
        ...
        if self.current_diagram == "structure" or self.last_results is None:
            ...
            u = self.units
            # Se reconstruyen coordenadas en unidades de pantalla para el
            # dibujo, y se convierten los resultados (que estan en SI) de
            # vuelta a unidades de pantalla para las etiquetas del diagrama.
